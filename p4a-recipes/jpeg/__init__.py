...
class JpegRecipe(Recipe):
    '''
    Local override of the upstream jpeg recipe.

    Fixes two failures seen with the current ``ghcr.io/kivy/buildozer:latest``
    image (CMake 4.x):

    * ``rm -f CMakeCache.txt CMakeFiles/`` cannot remove the ``CMakeFiles/``
      directory left by a previous build, so use ``-rf``.
    * CMake 4.x dropped compatibility with ``cmake_minimum_required`` < 3.5,
      which libjpeg-turbo's CMakeLists still declares; pass
      ``-DCMAKE_POLICY_VERSION_MINIMUM=3.5`` to configure anyway.
